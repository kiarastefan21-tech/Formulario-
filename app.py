...
def submit():
    ...
    sensores_dis = request.form.get("sensores_dis")
    ...
    # Tuberia Combustible 
    # Tubería Combustible
    cantidad_tuberias = request.form.get("cantidad_tuberias")

    if cantidad_tuberias == "" or cantidad_tuberias is None:
        cantidad_tuberias = 0
    else:
        cantidad_tuberias = int(cantidad_tuberias)

    reporte_tuberias = ""

    for i in range(1, cantidad_tuberias + 1):
        reporte_tuberias += f"""
        <h3>Tubería {i}</h3>

        <p><span class="label">Tipo:</span> {request.form.get(f"tuberia{i}_tipo_pipe")}</p>
        <p><span class="label">Tipo Tubería Combustible:</span> {request.form.get(f"tuberia{i}_tipo_com_pipe")}</p>
        <p><span class="label">Marca:</span> {request.form.get(f"tuberia{i}_marca_pipe")}</p>
        <p><span class="label">Fecha Instalación:</span> {request.form.get(f"tuberia{i}_fecha_pipe")}</p>
        <p><span class="label">Estado:</span> {request.form.get(f"tuberia{i}_estado_pipe")}</p>
        <p><span class="label">Geoducto:</span> {request.form.get(f"tuberia{i}_geo")}</p>
        <p><span class="label">Sump Bomba Sumergible:</span> {request.form.get(f"tuberia{i}_sumergible")}</p>
        <p><span class="label">Tipo Bomba Sumergible:</span> {request.form.get(f"tuberia{i}_tipo_sum")}</p>
        <p><span class="label">Hermeticidad:</span> {request.form.get(f"tuberia{i}_hermeticidad")}</p>
        <p><span class="label">Sump debajo Disp:</span> {request.form.get(f"tuberia{i}_disp_pipe")}</p>
        <p><span class="label">Tipo Disp:</span> {request.form.get(f"tuberia{i}_tipo_disp")}</p>
        <p><span class="label">Hermeticidad:</span> {request.form.get(f"tuberia{i}_hermeticidad1")}</p>
        <p><span class="label">Cajas a Prueba de Explosión:</span> {request.form.get(f"tuberia{i}_cajas")}</p>
        <p><span class="label">Comentarios:</span> {request.form.get(f"tuberia{i}_comentarios")}</p>
    ...
    for i in range(1, cantidad_dispensadores + 1):
        ...
        <p><span class="label">Productos:</span> {", ".join(request.form.getlist(f"disp{i}_productos"))}</p>
    ...
    <body>
        ...
        <p><span class="label">Sensores en Sump Dispensador:</span> {sensores_dis}</p>
        ...
        <div class="section">
            ...
            <p><span class="label">Cantidad de Tuberías:</span> {cantidad_tuberias}</p>
